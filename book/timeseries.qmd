...
if data_path.exists():
    ...
    ohlcv_pl = (
        pl.from_records(resp.json(), orient="row", schema=cols)
        .select(
            [
                pl.col(time_col)
                .cast(pl.Datetime)
...
ohlcv_pd = (
    ohlcv_pl.with_columns(pl.col("time").cast(pl.Datetime))
    .to_pandas()
    .set_index("time")
)
```
...
ohlcv_pl.select(
    [
        ...
        close.cumulative_eval(pl.element().mean()).alias("Expanding Average"),
...
Polars doesn't have an `expanding_mean` or `cum_mean` yet so we make do by combining `cumulative_eval` and `mean`.
