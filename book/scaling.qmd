...
Polars [launched its cloud offering in September 2025](https://pola.rs/posts/polars-cloud-launch/) with support for AWS, with on-premises support [announced in June 2026](https://pola.rs/posts/polars-distributed-available-on-kubernetes/).
However, at the time of writing, these are not free offerings, and open source Polars is still single-node only.

Polars does have a streaming mode for larger-than-memory datasets on a single machine.
It also uses memory more efficiently than Pandas.
These two things mean you can use Polars for much bigger data than Pandas can handle, and hopefully you won't need tools like Dask or Spark until you're actually running on a cluster.
...
:::{.callout-warning}
In older versions of Polars, streaming mode was enabled by passing `streaming=True` to `collect`, but in newer versions you should pass `engine="streaming"`.
The old way still works but is deprecated and will be removed in a future version.
:::
...
```
:::
The Polars code above tends to be ~7x faster than Dask on my machine. 

We should also profile memory usage, since it could be the case that Polars is just running faster because it's reading in bigger chunks.
Looking at peak resident memory, the Dask example peaks at ~3.0 GB, while Polars peaks at ~1.0 GB, so Polars wins on memory too. (You can dig into allocations in more detail with a profiler like [`memray`](https://github.com/bloomberg/memray).)
...
merged_pl = indiv_filtered_pl.join(cm_pl, on="CMTE_ID")
```
...
ax = (
    ...
    .select(
        [pl.col("TRANSACTION_DT"), pl.col(pl.Int32).rolling_mean(30, min_samples=0)]
    )
    .to_pandas()
    .set_index("TRANSACTION_DT")
    [["DEM", "REP"]]
...
The Polars team released a [benchmark comparing Polars to PySpark in June 2026](https://pola.rs/posts/polars-pyspark-benchmarks/), looking at single node and distributed performance on AWS.
In the single node benchmark Polars was ~6.4x faster than PySpark, ranging from 3x to 38x faster per query.
In the distributed benchmark Polars was ~3.2x faster than PySpark, ranging from 1.6x to 7.7x faster per query.