...
```
:::
The Polars code above tends to be ~10x faster than Dask on my machine.
...
According to the [`memray`](https://github.com/bloomberg/memray) profiler, the Dask example's memory usage peaks at ~4.0 GB, while Polars peaks at ~1.6 GB, so Polars wins on memory too.
