...
As you've probably heard by now, [Polars is very very fast](https://pola.rs/posts/benchmarks/).
...
dtypes_pl = (
    {col: pl.String for col in initially_str_cols_pl}
...
::: {.callout-tip}
Polars expressions have a [`shrink_dtype`](https://docs.pola.rs/api/python/stable/reference/expressions/api/polars.Expr.shrink_dtype.html) method that can be more convenient than manually specifying the dtypes yourself. It's not magic though, and it has to spend time finding the min and max of the column.
:::
...
There's not much that you haven't seen here already, so we won't explain the code line by line. The main new thing here is [`pl.when`](https://docs.pola.rs/api/python/stable/reference/expressions/api/polars.when.html) for ternary expressions.
...
def feet_to_cm_pl(col: pl.Expr) -> pl.Expr:
    ...
    ) + feet_inches_split.struct.field("field_1").str.strip_chars_end('"').cast(
...
def parse_suffixed_num_pd(col: pd.Series) -> pd.Series:
    suffix_value = (
        col.str[-1].map({"K": 1_000, "M": 1_000_000}).fillna(1).astype("uint32")
    )
    without_suffix = (
        col.str.replace("K", "", regex=False)
...
def parse_weight_pd(col: pd.Series) -> pd.Series:
    ...
    without_unit = (
        col.where(is_kg, col.str[:-3]).mask(is_kg, col.str[:-2]).astype("uint8")
    )
...
def parse_contract_pd(df: pd.DataFrame) -> pd.DataFrame:
    ...
    split = (
        contract_col.loc[contains_tilde]
    ...
    loan_date = parse_date_pd(not_tilde.loc[~free_agent].str[:-8]).rename(
        "loan_date_start"
    )
    return pd.concat(
        [df.drop("Contract", axis=1), split, free_agent, loan_date], axis=1
    )
```
...
In this example, **Polars is ~8x faster than Pandas**:
...
You could play around with the timings here and even try the [`.profile`](https://docs.pola.rs/api/python/stable/reference/lazyframe/api/polars.LazyFrame.profile.html) method to see what Polars spends its time on. In this scenario the speed advantage of Polars likely comes down to three things:
...
One use case for NumPy ufuncs is doing computations that Polars expressions don't support.
In this example Polars can do everything we need, and is significantly faster than the ufunc version.
In a previous of this book the ufunc version was slightly faster, but Polars performance is constantly improving.
...
This may not be a huge performance difference, but it at least means you don't sacrifice speed when relying on NumPy. There are some [gotchas](https://docs.pola.rs/user-guide/expressions/numpy-functions/#interoperability) though so watch out for those.
...
In a previous version of this book the Polars version was 10-100% slower than Pandas, but with the most recent (at time of writing) versions of both Polars is ~10% faster.
...
This example isn't telling you to use Pandas in this specific situation.
Once you add in the time spent reading a file, the Polars speedup likely increases.

And even here, if you sort by the `name` col, Polars wins by a larger margin. It has fast-track algorithms for sorted data.
...
- Polars works well with NumPy ufuncs, but prefer native Polars expressions when possible.
