...
In this example, **Polars is ~23x faster than Pandas**:
...
In a previous version of this book the ufunc version was slightly faster, but Polars performance is constantly improving.
...
Here the pure Polars version is several times faster, so you're generally better off reaching for native expressions. Still, it's good to know that NumPy ufuncs interoperate seamlessly for the cases where Polars doesn't have a built-in for what you need. There are some [gotchas](https://docs.pola.rs/user-guide/expressions/numpy-functions/#interoperability) though so watch out for those.
...
In a previous version of this book the Polars version was 10-100% slower than Pandas, but with the most recent (at time of writing) versions of both Polars is ~2.3x faster.
